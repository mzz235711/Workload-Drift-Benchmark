...
/*+ HashJoin(ph u p)
 ...
 SeqScan(p)
 Leading((ph (u p))) */
SELECT COUNT(*) FROM postHistory as ph, posts as p, users as u WHERE ph.PostId = p.Id AND p.OwnerUserId = u.Id AND ph.CreationDate>='2010-08-13 15:31:27'::timestamp AND p.PostTypeId=2 AND u.DownVotes<=1;
...
/*+ HashJoin(c u b)
 ...
 SeqScan(b)
 Leading(((c u) b)) */
SELECT COUNT(*) FROM comments as c, badges as b, users as u WHERE u.Id = c.UserId AND c.UserId = b.UserId AND u.DownVotes<=2 AND u.UpVotes>=0 AND u.UpVotes<=71;
...
/*+ HashJoin(ph c p)
 ...
 SeqScan(p)
 Leading((ph (c p))) */
SELECT COUNT(*) FROM comments as c, posts as p, postHistory as ph WHERE p.Id = c.PostId AND p.Id = ph.PostId AND c.Score=0 AND p.FavoriteCount>=0 AND p.FavoriteCount<=11;
...
/*+ HashJoin(c u b)
 ...
 SeqScan(b)
 Leading(((c u) b)) */
SELECT COUNT(*) FROM comments as c, badges as b, users as u WHERE u.Id = c.UserId AND c.UserId = b.UserId AND c.Score=0 AND u.Reputation>=1 AND u.Views>=0 AND u.Views<=30;
...
/*+ HashJoin(c pl v ph)
 ...
 IndexScan(ph)
 Leading(((c pl) (v ph))) */
SELECT COUNT(*) FROM comments as c, postLinks as pl, postHistory as ph, votes as v WHERE pl.PostId = c.PostId AND c.PostId = ph.PostId AND ph.PostId = v.PostId AND c.Score=0 AND c.CreationDate<='2014-09-10 16:30:40'::timestamp AND pl.CreationDate>='2010-11-07 21:23:52'::timestamp AND ph.PostHistoryTypeId=2 AND v.BountyAmount>=0;
