...
def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda, num_test_queries):
    ...
    dicts, column_min_max_vals, min_val, max_val, labels_train, labels_test, max_num_joins, max_num_predicates, train_data, test_data = get_train_datasets(
        workload_name, num_queries, num_materialized_samples)
    ...
    for i in range(num_test_queries):
        file_name = '/home_nfs/peizhi/zizhong/dataset/imdb/mscn/mscn_subquery/q' + str(i)
        ...
        filename = "/home_nfs/peizhi/zizhong/dataset/imdb/mscn/mscn_subquery_fore2e/q" + str(i) + "_card.csv"
        outfile = open(filename, 'w')
        for j, t in enumerate(tables):
            allperms = list(itertools.permutations(t))
            for perm in allperms:
                 for tmp in perm:
                     ali = table2alia(tmp)
                     outfile.write(ali + ',')
                 outfile.write(':' + str(preds_test_unnorm[j]) + '\n')
        outfile.close()
        # Write predictions
        #file_name = "results/predictions_" + workload_name + "/" + str(i) + "_card.csv"
...
def main():
    ...
    parser.add_argument("--test_queries", help="number of training queries (default: 10000)", type=int, default=800)
    ...
    train_and_predict(args.testset, args.queries, args.epochs, args.batch, args.hid, args.cuda, args.test_queries)
