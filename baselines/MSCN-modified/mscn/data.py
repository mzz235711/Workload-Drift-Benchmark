...
def load_data(train_query_file, train_bitmap_file, num_materialized_samples):
    joins = []
    predicates = []
    tables = []
    samples = []
    ...
    # Load queries
    with open(train_query_file, 'rU') as f:
        ...
        for row in data_raw:
            ...
            if int(row[3]) < 1:
                ...
                label.append(row[3])
    print("Loaded queries from {}".format(train_query_file))

    # Load bitmaps
    if num_materialized_samples > 0:
        num_bytes_per_bitmap = int((num_materialized_samples + 7) >> 3)
        with open(train_bitmap_file, 'rb') as f:
...
def load_and_encode_train_data(train_query_file, train_bitmap_file, save_folder, num_materialized_samples):
    #file_name_queries = "data/" + dataset + "/train"
    file_name_column_min_max_vals = save_folder +  "/column_min_max_vals.csv"

    joins, predicates, tables, samples, label = load_data(train_query_file, train_bitmap_file, num_materialized_samples)
    ...
    all_table_names = []
    with open(save_folder + "/tables.csv", 'rU') as f:
        data_raw = list(list(rec) for rec in csv.reader(f, delimiter=','))
    ...
    all_join_names = set()
    #join_file_name = "/joins_exp_paper.csv"
    join_file_name = "/joins.csv"
    with open(save_folder + join_file_name, 'rU') as f:
        data_raw = list(list(rec) for rec in csv.reader(f, delimiter=','))
        for i, row in enumerate(data_raw):
            if i == 0:
                continue
            join = [row[0].split('=')[0], row[0].split('=')[1]]
            join.sort()
            all_join_names.add(join[0] + "=" + join[1])
    all_join_names.add("")
    ...
    num_queries = len(joins)
    random_indices = np.arange(num_queries, dtype=int)
    np.random.shuffle(random_indices)
    num_train = int(num_queries * 0.9)
    num_test = num_queries - num_train

    #samples_enc = np.array(samples_enc)
    #predicates_enc = np.array(predicates_enc)
    #joins_enc = np.array(joins_enc)
    #label_norm = np.array(label_norm)

    #samples_train = samples_enc[random_indices[:num_train]]
    #predicates_train = predicates_enc[random_indices[:num_train]]
    #joins_train = joins_enc[random_indices[:num_train]]
    #labels_train = label_norm[random_indices[:num_train]]

    #samples_test = samples_enc[random_indices[num_train:num_train + num_test]]
    #predicates_test = predicates_enc[random_indices[num_train:num_train + num_test]]
    #joins_test = joins_enc[random_indices[num_train:num_train + num_test]]
    #labels_test = label_norm[random_indices[num_train:num_train + num_test]]

    samples_train = [samples_enc[i] for i in random_indices[:num_train]]
    predicates_train = [predicates_enc[i] for i in random_indices[:num_train]]
    joins_train = [joins_enc[i] for i in random_indices[:num_train]]
    labels_train = [label_norm[i] for i in random_indices[:num_train]]

    samples_test = [samples_enc[i] for i in random_indices[num_test:]]
    predicates_test = [predicates_enc[i] for i in random_indices[num_test:]]
    joins_test = [joins_enc[i] for i in random_indices[num_test:]]
    labels_test = [label_norm[i] for i in random_indices[num_test:]]
...
def make_dataset(samples, predicates, joins, labels, max_num_joins, max_num_predicates, robust=False):
    ...
    if robust is True:
        samples_tensors = sample_tensors * (np.random.rand(*sample_tensors.shape) > 0.2)
    ...
    if robust is True:
        predicate_tensors = predicate_tensors * (np.random.rand(*predicate_tensors.shape) > 0.2)
    ...
    if robust is True:
        join_tensors = join_tensors * (np.random.rand(*join_tensors.shape) > 0.2)
...
def get_train_datasets(train_query_file, train_bitmap_file, save_folder, num_materialized_samples, robust=False):
    dicts, column_min_max_vals, min_val, max_val, labels_train, labels_test, max_num_joins, max_num_predicates, train_data, test_data = load_and_encode_train_data(
        train_query_file, train_bitmap_file, save_folder, num_materialized_samples)
    train_dataset = make_dataset(*train_data, labels=labels_train, max_num_joins=max_num_joins,
                                 max_num_predicates=max_num_predicates, robust=robust)
