...
def unnormalize_labels(labels_norm, min_val, max_val, cuda=None):
    if cuda:
        labels_norm = [l.cpu().item() for l in labels_norm]
    else:
        labels_norm = [l.item() for l in labels_norm]
    #print(labels_norm)
...
def encode_data(predicates, joins, column_min_max_vals, column2vec, op2vec, join2vec):
    ...
    for i, query in enumerate(predicates):
        ...
        for predicate in joins[i]:
            # Join instruction
            if '=' in predicate:
                jt = [predicate.split('=')[0], predicate.split('=')[1]]
                jt.sort()
                p = jt[0] + '=' + jt[1]
            else:
                p = predicate
