...
def generate_bitmap(idx):
    sample_num = 1000
    conn = pg.connect(dbname='imdb', user='zizhongmeng', port=5434, host='localhost')
    cur = conn.cursor()
    #filename = './varying_value_distribution_queries/beta_distribution_2range/value_distribution_{}_subqueries/value_distribution_{}_subqueries_withcard'.format(idx, idx)
    #filename = 'value_distribution_1_1'
    filename = './varying_label_queries/{}_label_sub_queries/{}_label_sub_queries'.format(idx, idx)
    csv_file = open('{}.csv'.format(filename), 'r')
    #csv_file = open('{}/{}.csv'.format(folder, filename), 'r')
